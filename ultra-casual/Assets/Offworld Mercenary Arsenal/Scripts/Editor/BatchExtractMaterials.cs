...
public class BatchExtractMaterials : EditorWindow
{
	...
	private void DrawDestinationPathField()
	{
		...
		if (GUILayout.Button("Get Current Folder", GUILayout.Width(150)))
		{
			Type projectWindowUtilType = typeof(ProjectWindowUtil);
			MethodInfo getActiveFolderPath = projectWindowUtilType.GetMethod("GetActiveFolderPath", BindingFlags.Static | BindingFlags.NonPublic);
			object obj = getActiveFolderPath.Invoke(null, new object[0]);
			materialsFolder = obj.ToString();
	...
	private void DrawModelsToProcessList()
	{
		...
		style.margin = new RectOffset(5,5,1,5);
		GUILayout.BeginVertical("", style);

		var centeredStyle = GUI.skin.GetStyle("Label");
		centeredStyle.alignment = TextAnchor.MiddleCenter;
		GUILayout.Label("Models To Process (drag & drop here)", centeredStyle);
		...
		// Credit: https://answers.unity.com/answers/657877/view.html
		if ((ev.type == EventType.DragPerform || ev.type == EventType.DragUpdated) && verticalRect.Contains(ev.mousePosition))
		{
			DragAndDrop.visualMode = DragAndDropVisualMode.Copy;
			if (ev.type == EventType.DragPerform)
			{
		...
		if (GUILayout.Button("+", GL_WIDTH_25,GUILayout.Height(48)))
	...
	private void CalculateRemappedMaterials()
	{
		...
		for (int i = 0; i < modelData.Count; i++)
		{
			...
			UnityEngine.Object[] embeddedAssets = AssetDatabase.LoadAllAssetRepresentationsAtPath(modelPath);
